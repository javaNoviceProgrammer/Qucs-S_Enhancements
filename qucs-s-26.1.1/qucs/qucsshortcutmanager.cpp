...
QucsCommand::QucsCommand(const QString &id, const QString &category,
...
QucsShortcutManager::~QucsShortcutManager() {
  // Smart pointers are used. Cleanup is done automatically
}
...
void QucsShortcutManager::registerCommand(const QString &id,
  ...
  // Update shortcut index if there's a default key
  ...
  qDebug() << "Registered command:" << id << "Category:" << category
...
QList<QucsCommand *>
QucsShortcutManager::commandsInCategory(const QString &category) const {
...
bool QucsShortcutManager::setShortcut(const QString &commandId,
  ...
  qDebug() << "Shortcut changed:" << commandId << "from" << oldKey.toString()
           << "to" << key.toString();
...
QList<ShortcutConflict>
QucsShortcutManager::findConflicts(const QKeySequence &key) const {
...
bool QucsShortcutManager::saveToFile(const QString &filename) const {
  ...
  for (const QString &category : categories) {
    ...
    const auto commands = commandsInCategory(category);
...
bool QucsShortcutManager::loadFromFile(const QString &filename) {
  ...
  // Load shortcuts
  const auto shortcut_keys = shortcuts.keys();
  for (const QString &category : shortcut_keys) {
    QJsonObject categoryObj = shortcuts[category].toObject();

    const auto object_keys = categoryObj.keys();
    for (const QString &description : object_keys) {
